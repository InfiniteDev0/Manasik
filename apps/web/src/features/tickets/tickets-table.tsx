...
import { TicketsSelectionBar } from './tickets-selection-bar';
import { TicketsTabs, TicketsToolbar } from './tickets-toolbar';
  ...
  return {
    ...
    cell: ({ row }) => (
      <span className={cn('tabular-nums', id === 'net' ? 'text-foreground font-medium' : 'text-muted-foreground')}>
        {formatTicketMoney(row.original[id])}
      </span>
    ),
  };
}
...
const COLUMNS: ColumnDef<TicketRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="text-foreground">{formatTicketDate(row.original.date)}</span>,
  },
  {
    id: 'client',
    accessorKey: 'client',
  ...
  hiddenColumn('route', 'Route'),
  {
    ...
    cell: ({ row }) => (
      <span className="text-foreground">{row.original.departure ? formatTicketDate(row.original.departure) : '—'}</span>
    ),
  },
...
export function TicketsTable() {
  ...
  const tableData = React.useMemo(
    () =>
      data.filter((ticket) => (!category || category.matches(ticket)) && ticketMatchesDate(ticket, dateFilter)),
  ...
  return (
    <div className="flex flex-col gap-5">
      <TicketsHeader
        ...
        onAddTicket={() => setAdding((open) => !open)}
        adding={adding}
      />

      <div className="bg-card overflow-hidden rounded-xl border shadow-xs">
        <TicketsTabs value={categoryId} onValueChange={setCategoryId} counts={counts} />
        <TicketsToolbar
              ...
              {rows.map((row, index) => (
                <Reorder.Item
                  ...
                  className={cn(
                    ...
                    row.getIsSelected() ? 'bg-vivid-cyan/6' : index % 2 === 1 ? 'bg-muted/60' : 'bg-card',
                    'hover:bg-muted',
