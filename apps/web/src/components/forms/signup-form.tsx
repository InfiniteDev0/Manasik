...
// ─────────────────────────────────────────────────────────────────────────────
// UI ONLY — this form never creates an account.
//
// Manasik has a single admin account, created in the Supabase dashboard.
// While auth is switched off, "Create account" just opens the workspace, like
// "Log in" does. Once auth is on, it says sign-up isn't available — so flipping
// AUTH_ENABLED can never quietly open public sign-ups.
// ─────────────────────────────────────────────────────────────────────────────
...
interface SignupFormProps extends Omit<ComponentPropsWithoutRef<'form'>, 'onSubmit'> {
  ...
  onSwitchMode?: () => void;
  /** Where to land after "signing up" while auth is off. */
  next?: string;
}
...
// 5 checks: length, uppercase, lowercase, number, special character. If this
// form is ever wired to Supabase, set the same policy under Authentication →
// Passwords so the meter and the server agree.
...
export function SignupForm({ className, onSwitchMode, next, ...props }: SignupFormProps) {
  ...
  function handleSubmit(e: SubmitEvent<HTMLFormElement>) {
    ...
    // The form's own checks still run, so the flow feels real.
    ...
    if (AUTH_ENABLED) {
      gooeyToast.error('Sign-up isn’t available', {
        description: 'Log in with your admin account.',
      });
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className={cn('w-full max-w-md', className)} {...props}>
      ...
      <button
        type="submit"
        // Disabled until terms are accepted — an obvious blocker beats a
        // rejected submit.
      ...
      {/* Federated — inert until OAuth lands.
      <button
        type="button"
        disabled
